...
import {alignCenter, alignLeft, alignRight, verticalAlignBottom, verticalAlignCenter, verticalAlignTop} from "../icons";
	...
	render() {
		...
		return (
			<Fragment>
				<InspectorControls>
					<PanelBody
						title={__('Accordion items Options', 'wp-bootstrap-blocks')}
						initialOpen={true}
					>
						<CheckboxControl
...
const applyWithDispatch = withDispatch((dispatch, {setAttributes, items, clientId}, registry) => {
	return {
		updateAlwaysOpen(alwaysOpen) {
			...
			items.forEach((item) => {
				updateBlockAttributes(item.clientId, {alwaysOpen, parentClientId: clientId})
			})
		}
	};
});
