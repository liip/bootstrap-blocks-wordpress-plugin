...
context('Row Block', () => {
	...
	it( 'Should be possible to select custom template', () => {
		...
		cy.get('.wp-bootstrap-blocks-template-selector-button > button[aria-label="Custom"]').click()
		cy.get('.wp-block-wp-bootstrap-blocks-row > .block-editor-inner-blocks > .block-editor-block-list__layout > .block-list-appender').should('exist')
		cy.postContentMatchesSnapshot()
	} );
	...
	it( 'Should only be possible to add column in custom layout', () => {
		...
		cy.get('.wp-block-wp-bootstrap-blocks-row > .block-editor-inner-blocks > .block-editor-block-list__layout > .block-list-appender > button').click()

		cy.get('.block-editor-block-list__block[data-type="wp-bootstrap-blocks/column"]').should( 'have.length', 3 );
	} );
	...
	it( 'Should be possible to enable column layout in editor', () => {
		...
		cy.get('.block-editor-block-list__block[data-type="wp-bootstrap-blocks/row"][data-editor-stack-columns="true"]').should('have.length',1 );
		cy.postContentMatchesSnapshot()
	} );
