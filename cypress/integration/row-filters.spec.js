...
describe( 'Row Block Filters', () => {
	...
	it( 'wpBootstrapBlocks.row.templates should add additional template', () => {
		...
		cy.get('.wp-bootstrap-blocks-template-selector-button').should('have.length',5 ); // 4 default templates + 1 additional template (custom template disabled)
		cy.get('.wp-bootstrap-blocks-template-selector-button > button[aria-label="1 Column (2/3 width)"]').should('exist');
		cy.get('.wp-bootstrap-blocks-template-selector-button > button[aria-label="1 Column (2/3 width)"] > span.dashicons-yes').should('exist');

		// Template should be applied
		cy.get('.wp-bootstrap-blocks-template-selector-button > button[aria-label="1 Column (2/3 width)"]').click();
	...
	it( 'wpBootstrapBlocks.row.enableCustomTemplate should disable custom template', () => {
		...
		cy.get('.wp-bootstrap-blocks-template-selector-button').should('have.length', 5 ); // 4 default templates + 1 additional template (custom template disabled)
		cy.get('.wp-bootstrap-blocks-template-selector-button > button[aria-label="Custom"]').should('not.exist');
	} );
	...
	it( 'wp_bootstrap_blocks_row_default_attributes should override default attributes', () => {
		...
		cy.get('.wp-bootstrap-blocks-template-selector-button > button[aria-label="2 Columns (1:2)"].is-active').should('exist');
		...
		cy.getCheckboxByLabel( 'Editor: Display columns stacked' ).should('be.checked');
