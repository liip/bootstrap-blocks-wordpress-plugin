...
describe( 'Column Block Filters', () => {
	...
	it( 'wpBootstrapBlocks.column.bgColorOptions should add background color', () => {
		...
		cy.get('.components-circular-option-picker__option[aria-label="Color: brand"]').should('exist');

		// Background color should be applied
		cy.get('.components-circular-option-picker__option[aria-label="Color: brand"]').click();
	...
	it( 'wp_bootstrap_blocks_column_default_attributes should override default attributes', () => {
		...
		cy
			.get('.block-editor-block-list__block[data-type="wp-bootstrap-blocks/column"]')
			.first()
			.should('have.attr', 'data-bg-color', 'primary')

		cy.getCheckboxByLabel( 'Center content vertically in row' ).should('be.checked');
