...
describe( 'Column Block', () => {
	...
	it( 'Column block should not be available in block inserter', () => {
		...
		cy.window().then( ( window ) => {
			const noResultPreWP55 = window.document.querySelector('.block-editor-inserter__no-results')
			const noResultWP55 = window.document.querySelector('.block-editor-inserter__content .has-no-results');
	...
	it( 'Column block should be initialized with default attributes', () => {
		...
		cy
			.get('[data-type="wp-bootstrap-blocks/column"]')
			.first()
	...
	it( 'Should be possible to change column size', () => {
		...
		cy.get('input[aria-label="Lg Column count"][disabled]').should('exist')

		cy.postContentMatchesSnapshot()

		cy.get('input.components-input-control__input[aria-label="Xl Column count"]').type('2');
		cy
			.get('[data-type="wp-bootstrap-blocks/column"]')
			.first()
	...
	it( 'Should not display xxl breakpoint options if run with Bootstrap 4', () => {
		...
		cy.xpath('//label[contains(@class,"components-base-control__label")][contains(text(),"Xl Column count")]').should('exist');

		// Xxl column count option should not exist
		cy.xpath('//label[contains(@class,"components-base-control__label")][contains(text(),"Xxl Column count")]').should('not.exist');

		// Xl equal-width option should exist
		cy.xpath('//label[contains(@class,"components-checkbox-control__label")][contains(text(),"Xl equal-width")]').should('exist');

		// Xxl equal-width option should not exist
		cy.xpath('//label[contains(@class,"components-checkbox-control__label")][contains(text(),"Xxl equal-width")]').should('not.exist');
	} );
} );
