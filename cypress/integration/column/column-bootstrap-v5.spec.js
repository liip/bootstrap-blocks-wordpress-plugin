...
describe( 'Column Block Bootstrap 5', () => {
	...
	it( 'Should display xxl breakpoint options', () => {
		...
		cy.xpath('//label[contains(@class,"components-base-control__label")][contains(text(),"Xxl Column count")]').should('exist');

		// Xxl equal-width option should exist
		cy.xpath('//label[contains(@class,"components-checkbox-control__label")][contains(text(),"Xxl equal-width")]').should('exist');
	} );
	...
	it( 'Should be possible to select column size for xxl breakpoint', () => {
		...
		// Change column count
		cy.get('input.components-input-control__input[aria-label="Xxl Column count"]').type('2');
		cy
